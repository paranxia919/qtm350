...
- Finally, we learnt about [Dask Clusters](https://docs.dask.org/en/latest/setup/clusters.html), which allow you to scale your computations across multiple cores or machines
:::
...
:::{.column width="50%"}
 ...
 Now we just need to activate the environment:
...
- Other software like [Singularity](https://sylabs.io/singularity/) is used in more niche applications, such as high-performance computing, but has a smaller user base
...
- It is just a text file that you can put under version control
- Then we need to use the Dockerfile to generate a Docker image
